...
const UpdateMail = () => {
  ...
  // Ensure `token` is a string, not a string[] (use first element if it's an array)
  const token = Array.isArray(params?.token) ? params.token[0] : params?.token;

  // If no token, redirect and prevent further actions
  useEffect(() => {
    if (!token) {
      router.push('/login');
    }
  ...
  const handleUpdateMail = async (values: any, resetForm: any) => {
    ...
    try {
      // Ensure token is a string before passing it to the fetch function
      if (!token) {
        throw new Error('Token is missing.');
      }

      const { result, ok } = await updateMailFetchDb({
        oldEmail,
        newEmail,
        token,
      });
  ...
  // Return loading or form based on token availability
  if (!token) return null; // Optional: Return a loading state or nothing while redirecting
