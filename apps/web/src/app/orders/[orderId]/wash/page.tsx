...
export default function WasherStation() {
    ...
    const handleConfirm = async (isConfirmed:any) => {
        setShowConfirmation(false);
        if (!isConfirmed) return;

        if (JSON.stringify(orderItems) !== JSON.stringify(orderItemReference)) {
            setNotMatchConfirmation(true);
            return;
        }

        // lanjutkan proses update status order dan kirim notifikasi
        try {
    ...
    const handleMismatchConfirm = (recalculate:any) => {
        setNotMatchConfirmation(false);
        if (!recalculate) {
            // Tambahkan logika untuk melapor ke admin jika diperlukan
        }
    };
